...
def process_compound_set_job_file(task_params):
    ...
    sdf_file = None
    try:
        sdf_file = process_compound_set_file(jr_id, transition_time)
    except RuntimeError as error:
...
def erase_compound_set_job_material(task_params, job_request_id=0):
    ...
    parameters suitable for validation
    """
    del task_params  # Unused, present to permit task linking.
    ...
    code = str(job_request.code)
    logger.info('Erasing upload material (%s) code=%s', job_request_id, code)
