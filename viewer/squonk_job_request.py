...
def create_squonk_job(request):
    ...
    if not job_transfers:
        logger.warning(
            'No JobFileTransfer object for snapshot %s',
            snapshot_id,
        )
        raise ValueError(
    ...
    if job_transfer.transfer_status != 'SUCCESS':
        logger.warning(
            'JobFileTransfer status is not SUCCESS (is %s)',
    ...
    if not sq2_rv.success:
        msg = (
            f'JobTransfer failed to get/create a Squonk2 Project'
    ...
    logger.info('"%s" callback_url=%s', job_name, callback_url)
    logger.info('"%s" callback_token=%s', job_name, callback_token)
    logger.info('"%s" squonk_job_spec=%s', job_name, squonk_job_spec)

    # Dry-run the Job execution (this provides us with the 'command', which is
    # placed in the JobRecord's squonk_job_info).
    logger.info(
        '"%s" code=%s calling DmApi.dry_run_job_instance()',
        job_name,
        job_request.code,
    )
    ...
    logger.info('"%s" result=%s', job_name, result)

    if not result.success:
        logger.warning(
            '"%s" code=%s dry_run_job_instance() FAILED result=%s',
            job_name,
            job_request.code,
            result,
        )
    ...
    logger.info(
        '"%s" code=%s calling DmApi.start_job_instance()',
        job_name,
        job_request.code,
    )
    ...
    logger.info(
        '"%s" code=%s SUCCESS (job_instance_id=%s job_task_id=%s)',
        job_name,
        job_request.code,
        job_instance_id,
        job_task_id,
    ...
    # If we are not expecting callbacks from Squonk then we can safely set the
    # JobRequest record's squonk_url_ext value. Otherwise there are
    # race conditions.
    if settings.SQUONK2_REFRESH_SHOULD_RETRIEVE_RESULTS:
        logger.warning(
            '"%s" Setting squonk_url_ext (%s). Normally set in the callback',
            job_name,
            squonk_url_ext,
        )
        job_request.squonk_url_ext = squonk_url_ext
        job_request.save()
